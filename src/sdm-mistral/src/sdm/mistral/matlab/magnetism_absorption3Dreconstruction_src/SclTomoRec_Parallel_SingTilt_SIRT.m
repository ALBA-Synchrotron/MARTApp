%Function for the Reconstruction of the Electronic Charge (Structure) of a
%single Tilt series using the SIRT algorithm.
%--------------------------------------------------------------------------
%Function Inputs
%
%   Chg_Tomo:   Images Stack containing the Tilt series associated with the
%               sum of log(C+) and log(C-). These will contain the charge
%               and XMLD contribution of the projections allowing to
%               reconstruct the electronic density (Structure).
...
%Function Outputs
%
%   charge_Mod: Tensor with the dimensions indicated by Mod_Size containing
%               the reconstructed attenuation lengths which give rise to
%               the charge contrast term.
%
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
function [charge_Mod] = SclTomoRec_Parallel_SingTilt_SIRT(Chg_Tomo,Pix_Size,Tilt_v,Mod_Size,N_iter,LC_Flag,Recon_Flag,SimultSlcs,GPU_Flag,Use_CalMat,SaveCalMat)
...
Det_Size = size(Chg_Tomo);
%Reshaping to be DetSize_X, DetSize_Y, DetSize_Z (Z projections)
Det_Size =[Det_Size(2),Det_Size(1),Det_Size(3)];
%Images Arrangement in Vector.
if strcmp(Recon_Flag,'XTilt') == 1 %Creation of the sinogram for Y slices.
    ...
    Chg_Tomo_v = zeros(Det_Size(2)*SimultSlcs*n_Proj,Det_Size(1)/SimultSlcs);
    ...
    for ColBlock=1:Det_Size(1)/SimultSlcs
        for pj=1:n_Proj
            Chg_Tomo_v((pj-1)*Det_Size(2)*SimultSlcs+1:pj*Det_Size(2)*SimultSlcs,ColBlock) = reshape(Chg_Tomo(:,(ColBlock-1)*SimultSlcs+1:ColBlock*SimultSlcs,pj),[Det_Size(2)*SimultSlcs,1])./2; %The 2 factor is assuming that you are using Cp + Cm as input data for the Charge Contribution. Otherwise, the 2 factor is incorrect.
        end
    end
elseif strcmp(Recon_Flag,'YTilt') == 1 %Creation of the sinogram for each X slice.
    %Checking the Number of Slices of the data matches the length of the
    %data.
    ...
    Chg_Tomo_v = zeros(Det_Size(1)*SimultSlcs*n_Proj,Det_Size(2)/SimultSlcs);
    ...
    for RowBlock=1:Det_Size(2)/SimultSlcs
        for pj=1:n_Proj
            Chg_Tomo_v((pj-1)*Det_Size(1)*SimultSlcs+1:pj*Det_Size(1)*SimultSlcs,RowBlock) = reshape(Chg_Tomo((RowBlock-1)*SimultSlcs+1:RowBlock*SimultSlcs,:,pj)',[Det_Size(1)*SimultSlcs,1])./2;
        end
    end
end
clear('Chg_Tomo');
if GPU_Flag == 0 %No GPU Implementation.
    if LC_Flag == 0
        ...
        Chg_Mod_v = zeros(Mod_Size(1)*Mod_Size(2)*Mod_Size(3),1);
        if Use_CalMat == 0
            %---------------------
                ...
                for it=1:N_iter
                    if it == 1 %Saving the projection matrices in the HDD to save execution time.
                        if slc == 1 && SaveCalMat == 1
                            mkdir('Scl_Projection_Matrices')
                        else
                        end
                        if SaveCalMat == 1 %Save the projection matrices.
                            path_mat = pwd;
                            path_mat = [path_mat,'\Scl_Projection_Matrices\'];
                            [Fwd_Proj_Mat,R_Mat,C_Mat] = SclTomo_Fwd_Mat_Parallel_SingTiltSIRT(Det_Size,[Fast_Idx,slc],Pix_Size,Mod_Size,Tilt_v,Recon_Flag,SimultSlcs,LC_Flag);
                    ...
                    err_v = Chg_Tomo_v(:,slc)-Fwd_Proj_Mat*(Chg_Mod_v);
                    Chg_Mod_v = Chg_Mod_v + Bkwd_Proj_Mat*err_v;
                    fprintf('Slice %i of %i. Completed Iteration %i of %i.\n',slc,Slice_Count,it,N_iter)
                end
            end
            %Saving Temporary Data
            N_iter_comp = N_iter;
            save('Temp_Recon.mat','Chg_Mod_v','Mod_Size','ext_Mod','Recon_Flag','SimultSlcs','N_iter_comp');
        elseif Use_CalMat == 1
            path_mat = pwd;
            path_mat = [path_mat,'\Scl_Projection_Matrices\'];
            %---------------------
    ...
    elseif LC_Flag == 1
        ...
        if strcmp(Recon_Flag,'XTilt') == 1
            Chg_Mod_v = zeros(Mod_Size(1)*(Mod_Size(2)+2*ext_Mod)*Mod_Size(3),1);
        elseif strcmp(Recon_Flag,'YTilt') == 1
            Chg_Mod_v = zeros((Mod_Size(1)+2*ext_Mod)*Mod_Size(2)*Mod_Size(3),1);
        else
        end
        if Use_CalMat == 0
            %---------------------
                ...
                for it=1:N_iter
                    if it == 1 %Saving the projection matrices in the HDD to save execution time.
                        ...
                        if SaveCalMat == 1
                            path_mat = pwd;
                            path_mat = [path_mat,'\Scl_Projection_Matrices\'];
                            [Fwd_Proj_Mat,R_Mat,C_Mat] = SclTomo_Fwd_Mat_Parallel_SingTiltSIRT(Det_Size,[Fast_Idx,slc],Pix_Size,Mod_Size,Tilt_v,Recon_Flag,SimultSlcs,LC_Flag,max_Ang);
                            Bkwd_Proj_Mat = C_Mat*Fwd_Proj_Mat'*R_Mat;
                            save(sprintf('%sSlice_%i.mat',path_mat,slc),'Fwd_Proj_Mat','R_Mat','C_Mat','-v7.3');
                            clear('C_Mat','R_Mat');
                            fprintf('Calculated and saved Reconstruction Matrices of Slice %i of %i.\n',slc,Slice_Count)
...
elseif GPU_Flag == 1 %GPU Implementation
    Chg_Tomo_v = gpuArray(Chg_Tomo_v);
    if LC_Flag == 0
        ext_Mod = 0;
        %Models Initialization
        Chg_Mod_v = gpuArray(zeros(Mod_Size(1)*Mod_Size(2)*Mod_Size(3),1));
        if Use_CalMat == 0
            %---------------------
            %Scalar Reconstruction
            %---------------------
            for slc=1:Slice_Count
                for it=1:N_iter
                    ...
                    err_v = Chg_Tomo_v(:,slc)-Fwd_Proj_Mat_GPU*Chg_Mod_v;
                    Chg_Mod_v = Chg_Mod_v + Bkwd_Proj_Mat_GPU*err_v;
                    fprintf('Slice %i of %i. Completed Iteration %i of %i.\n',slc,Slice_Count,it,N_iter)
                end
            end
            %Saving Temporary Data
    ...
    elseif LC_Flag == 1
        ...
        %Model Initialization
        if strcmp(Recon_Flag,'XTilt') == 1
            Chg_Mod_v = gpuArray(zeros(Mod_Size(1)*(Mod_Size(2)+2*ext_Mod)*Mod_Size(3),1));
        elseif strcmp(Recon_Flag,'YTilt') == 1
            Chg_Mod_v = gpuArray(zeros((Mod_Size(1)+2*ext_Mod)*Mod_Size(2)*Mod_Size(3),1));
        else
        end
        if Use_CalMat == 0
            %---------------------
...
%Output Arrangement.
if LC_Flag == 0
    charge_Mod = zeros(Mod_Size(2),Mod_Size(1),Mod_Size(3));
    for k=1:Mod_Size(3)
        for i=1:Mod_Size(2)
            charge_Mod(i,:,k) = Chg_Mod_v(1+(i-1)*Mod_Size(1)+(k-1)*Mod_Size(1)*Mod_Size(2):...
...
elseif LC_Flag == 1
    if strcmp(Recon_Flag,'XTilt') == 1
        ...
        charge_Mod = zeros(Mod_SizeAr(2),Mod_SizeAr(1),Mod_SizeAr(3));
    elseif strcmp(Recon_Flag,'YTilt') == 1
    ...
    for k=1:Mod_SizeAr(3)
        for i=1:Mod_SizeAr(2)
            charge_Mod(i,:,k) = Chg_Mod_v(1+(i-1)*Mod_SizeAr(1)+(k-1)*Mod_SizeAr(1)*Mod_SizeAr(2):...
                Mod_SizeAr(1)+(i-1)*Mod_SizeAr(1)+(k-1)*Mod_SizeAr(1)*Mod_SizeAr(2))';
        end
    end
    if strcmp(Recon_Flag,'XTilt') == 1
        charge_Mod = charge_Mod(ext_Mod+1:Mod_Size(2)+ext_Mod,:,:);
    elseif strcmp(Recon_Flag,'YTilt') == 1
        charge_Mod = charge_Mod(:,ext_Mod+1:Mod_Size(1)+ext_Mod,:);
    else
    end
else
    fprintf('Error. LC_Flag must be 1(activated) or 0(deactivated).\n')
