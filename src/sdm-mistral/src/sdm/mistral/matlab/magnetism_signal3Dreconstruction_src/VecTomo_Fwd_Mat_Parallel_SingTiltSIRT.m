...
%specific tilt angle in order to apply the SIRT algorithm. In the Scalar
%field reconstruction, a Ytilt series is assumed, while in the Vector case
%Ytilt and Xtilt are taken into account. CPU Parallelized.
%--------------------------------------------------------------------------
%Function Inputs
...
%               reconstruction problem. The flags are 'XTiltVector' or
%               'YTiltVector'.
...
%                   dimensions 2*Number_Pixels_Det x 3*Number_Model_Cells
%                   for the Vector case. The matrix is compatible with the
...
%                   first and after through i. "x" is composed by the
%                   two concatenated column vectors representing m_inplane
%                   and m_outofplane magnetization components arranged in
%                   the same raster order. "x" =
%                   [m1(m_i,m_j,m_k);m2(m_i,m_j,m_k)].
%                   The Units of the projection matrix are expressed in
%                   pixels. To convert to real units it is necessary to
...
%--------------------------------------------------------------------------
function [Fwd_Proj_Mat,R_Mat,C_Mat] = VecTomo_Fwd_Mat_Parallel_SingTiltSIRT(Recon_Mask,Det_Size,Det_Data,Pix_Size,Mod_Size,Tilt_Ang,Recon_Flag,SimultSlcs,LC_Flag,varargin)
Use_Mask = length(unique(Recon_Mask)) > 1;

%Flag Check
if strcmp(Recon_Flag,'XTiltVector') == 0 && strcmp(Recon_Flag,'YTiltVector') == 0
    fprintf('Error.\nThe Recon_Flag input argument must be XTiltVector, YTiltVector only.')
    Fwd_Proj_Mat = nan(1);
    R_Mat = nan(1);
    C_Mat = nan(1);
    return
...
if strcmp(Recon_Flag,'XTiltVector') == 1
    ...
    parfor i=1:Data_n_Pix
        ...
        if isempty(len_v) == 1
            ...
            %Model Constrain Comment if do not want to do a Constrain.
            %Test
            if Use_Mask
                for i1=1:numel(len_v)
                    if Recon_Mask(y_idx(i1),x_idx(i1),z_idx(i1))~=1
                        len_v(i1)=0;
                    else
                    end
                end
    ...
    [Fwd_Proj_Mat,R_Mat,C_Mat] = Proj_Mat_Arrang_SIRT([Det_Data,n_Proj],SimultSlcs,Mod_SizeAr,'XTiltVector',Pix_Size,Det_idx_Cell_Xt,Model_idx_Cell_Xt,Length_Cell_Xt,Ang_Cell_Xt);
    %YTiltVector
elseif strcmp(Recon_Flag,'YTiltVector') == 1
    Data_n_Pix = Det_Data(1)*n_Proj*SimultSlcs;
    Fast_Idx = Det_Data(1)*SimultSlcs;
    ...
    [Fwd_Proj_Mat,R_Mat,C_Mat] = Proj_Mat_Arrang_SIRT([Det_Data,n_Proj],SimultSlcs,Mod_SizeAr,'YTiltVector',Pix_Size,Det_idx_Cell_Yt,Model_idx_Cell_Yt,Length_Cell_Yt,Ang_Cell_Yt);
else
end
