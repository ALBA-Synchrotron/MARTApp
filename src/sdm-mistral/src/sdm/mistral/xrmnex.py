...
import sdm.mistral
from sdm.mistral.parser import get_db, get_file_paths
        ...
        for date_sample_energie in dates_samples_energies:
            ...
            if not organize_by_repetitions:
                ...
                for zpz in zpz_positions_by_sample_e:
                    ...
                    sorted_fn_by_zpz_query = sorted(
                        fn_by_zpz_query, key=itemgetter("angle")
                    )

                    files = get_file_paths(
                        sorted_fn_by_zpz_query,
                        root_path,
            ...
            files_FF = get_file_paths(
                query_output, root_path, use_subfolders=use_subfolders
            )
...
class validate_getter(object):
    ...
    def __call__(self, method):
        def wrapped_method(xradia_file, *args, **kwargs):
            ...
            return method(xradia_file, *args, **kwargs)
...
class XradiaFile(object):
    ...
    def _get_axes_positions(self, n_images=None):
        if self._axes_positions is None:
            ...
            struct_fmt = "<%df" % number_of_floats
            self._axes_positions = struct.unpack(struct_fmt, data)
        if n_images is None:
            n_images = self.no_of_images
        return self._axes_positions[: self.no_of_axes * n_images]

    def get_axes_positions(self):
        return self._get_axes_positions(n_images=1)

    axes_positions = property(get_axes_positions)

    def get_position(self, name):
        index = self.axes_names.index(name)
        return self.axes_positions[index]

    def get_positions(self, name):
        axis_positions = self._get_axes_positions()
        offset = self.axes_names.index(name)
        for i in range(self.no_of_images):
            yield axis_positions[self.no_of_axes * i + offset]

    def get_sample_distance(self):
        return self.get_position("Sample Z")

    sample_distance = property(get_sample_distance)

    def get_detector_distance(self):
        return self.get_position("Detector Z") * 1000  # from mm to um

    detector_distance = property(get_detector_distance)

    def get_distance(self):
        if (
            self.sampleenc_name == "Sample Z"
            and self.detectorenc_name == "Detector Z"
        ):
            distance = (
                self.det_zero + self.detector_distance + self.sample_distance
            )
        return distance
    ...
    def get_image(self):
        ...
        image = np.flipud(
            np.reshape(
                imgdata, (self.image_height, self.image_width), order="A"
            )
        )
        image = np.reshape(
            image, (1, self.image_height, self.image_width), order="A"
        )
        return image
    ...
    def get_axes_names(self):
        if self._axes_names is None:
            ...
            self._axes_names = re.split("\s+\s+", axis_names_raw)  # noqa: W605
    ...
    def check_name(self, name):
        if name not in self.axes_names:
            raise LookupError
        return name
    ...
    def get_energyenc_name(self):
        name = None
        try:
            name = self.check_name("Energyenc")
        except LookupError:
            pass
        return name
    ...
    def get_energy_name(self):
        return self.check_name("Energy")

    energy_name = property(get_energy_name)

    def get_detectorenc_name(self):
        return self.check_name("Detector Z")

    detectorenc_name = property(get_detectorenc_name)

    def get_sampleenc_name(self):
        return self.check_name("Sample Z")

    sampleenc_name = property(get_sampleenc_name)

    def get_current_name(self):
        return self.check_name("machine_current")

    current_name = property(get_current_name)

    def get_polarisation_name(self):
        return self.check_name("polarisation")

    polarisation_name = property(get_polarisation_name)
    ...
    def get_no_of_images(self):
        if self._no_of_images is None:
            ...
            self._no_of_images = int(nimages[0])
    ...
    def get_image_width(self):
        if self._image_width is None:
            ...
            self._image_width = int(yimage[0])
    ...
    def get_image_height(self):
        if self._image_height is None:
            ...
            data = stream.read()
            yimage = struct.unpack("<I", data)
            self._image_height = int(yimage[0])
        return self._image_height

    image_height = property(get_image_height)

    def get_machine_currents(self):
        return list(self.get_positions(self.current_name))  # In mA

    def get_polarisation(self):
        return list(self.get_positions("polarisation"))

    @validate_getter([])
    def get_energies(self):
        if self.energyenc_name:
            energies = list(self.get_positions(self.energyenc_name))  # In mA

        # Energy for each image calculated from Energy motor ####
        elif self.energy_name:
            energies = list(self.get_positions(self.energy_name))  # In eV
        ...
        # Energy for each image calculated from ImageInfo ####
            ...
            try:
                # we found some txrm images (flatfields) with different
                # encoding of data
                energies = struct.unpack(struct_fmt, data)
            except struct.error:
                print(
                    "Unexpected data length (%i bytes). "
            ...
            raise RuntimeError(
                "There is no information about the energies at"
    ...
    def get_exp_times(self):
        ...
        struct_fmt = "<{0:10}f".format(self.no_of_images)
        try:
            # we found some txrm images (flatfields) with different encoding
            # of data
    ...
    def get_single_date(self):
        ...
        year_head = "20"
        if date.decode()[-1] == ":":
            date = struct.unpack("<" + "19s21x", data)[0]
            year_head = ""

        startdate = date.decode()
        ...
        raw_time = datetime.datetime(year, month, day, hour, minute, second)
    ...
    def get_dates(self):
        if self._dates is None:
            stream = self.file.openstream("ImageInfo/Date")
            data = stream.read()
            self._dates = struct.unpack(
                "<" + "17s23x" * self.no_of_images, data
            )
            if self._dates[0].decode()[-1] == ":":
                self._dates = struct.unpack(
                    "<" + "19s21x" * self.no_of_images, data
                )
        return self._dates
    ...
    def get_end_date(self):
        enddate = self.dates[self.no_of_images - 1].decode()
...
class xrmNXtomo(object):
    ...
    def __init__(
        ...
        self.program_version = sdm.mistral.__version__
        self.title = title
    ...
    def convert_metadata(self):
        ...
        self.nxdetectorsample.create_dataset("x_pixel_size", data=pixel_size)
        self.nxdetectorsample.create_dataset("y_pixel_size", data=pixel_size)
        ...
        # h5py NeXus link
        source_addr = bytes("/NXtomo/sample/rotation_angle", encoding="utf-8")
    ...
    def _convert_samples(self):
        ...
        self.nxdetectorsample.create_dataset(
            "data",
            shape=(self.nSampleFrames, self.numrows, self.numcols),
            chunks=(1, self.numrows, self.numcols),
            dtype=self.datatype,
        )

        self.nxdetectorsample["data"].attrs["Data Type"] = self.datatype
        self.nxdetectorsample["data"].attrs[
            "Number of Frames"
        ] = self.nSampleFrames
        self.nxdetectorsample["data"].attrs["Image Height"] = self.numrows
        self.nxdetectorsample["data"].attrs["Image Width"] = self.numcols
        ...
        # h5py NeXus link
        source_addr = bytes("/NXtomo/instrument/sample/data", encoding="utf-8")
        target_addr = bytes("data", encoding="utf-8")
        self.nxdetectorsample["data"].attrs["target"] = source_addr
        self.nxdata._id.link(source_addr, target_addr, h5py.h5g.LINK_HARD)
    ...
    def _convert_bright(self):
        ...
        self.nxbright.create_dataset(
            ...
            chunks=(1, self.numrows_bright, self.numcols_bright),
            dtype=self.datatype_bright,
        )
        self.nxbright["data"].attrs["Data Type"] = self.datatype_bright
        self.nxbright["data"].attrs["Image Height"] = self.numrows_bright
        self.nxbright["data"].attrs["Image Width"] = self.numcols_bright

        for numimage in range(self.nFramesBright):
            if numimage + 1 == self.nFramesBright:
                print(
                    (
                        "%i Bright-Field images "
                        "converted\n" % self.nFramesBright
                    )
                )
            self.count_num_sequence = self.count_num_sequence + 1
    ...
    def _convert_zero_deg_images(self, ole_zerodeg):
        ...
        if (
            ole_zerodeg.exists("ImageInfo/NoOfImages")
            and ole_zerodeg.exists("ImageInfo/ImageWidth")
            and ole_zerodeg.exists("ImageInfo/ImageHeight")
        ):

            stream = ole_zerodeg.openstream("ImageInfo/ImageHeight")
            data = stream.read()
            yimage = struct.unpack("<I", data)
            self.numrows_zerodeg = int(yimage[0])
            if verbose:
                print("ImageInfo/ImageHeight = %i" % yimage[0])
            stream = ole_zerodeg.openstream("ImageInfo/ImageWidth")
            data = stream.read()
            ximage = struct.unpack("<I", data)
            self.numcols_zerodeg = int(ximage[0])
            if verbose:
                print("ImageInfo/ImageWidth = %i" % ximage[0])
        else:
            print(
                "There is no information about the 0 degrees image size "
        ...
        if ole_zerodeg.exists("ImageData1/Image1"):
            ...
            if self.datatype == "uint16":
                struct_fmt = "<{0:10}H".format(
                    self.numrows_zerodeg * self.numcols_zerodeg
                )
                imgdata = struct.unpack(struct_fmt, data)
            ...
            imgdata_zerodeg = np.flipud(
                np.reshape(imgdata, (self.numrows, self.numcols), order="A")
            )
        else:
            imgdata_zerodeg = 0
    ...
    def convert_tomography(self):
        ...
        if self.filename_zerodeg_in is not None:
            ...
            self.nxdetectorsample["0_degrees_initial_image"].attrs[
                "Data Type"
            ] = self.datatype_zerodeg
            self.nxdetectorsample["0_degrees_initial_image"].attrs[
                "Image Height"
            ] = self.numrows_zerodeg
            self.nxdetectorsample["0_degrees_initial_image"].attrs[
                "Image Width"
...
class xrmReader(object):
    ...
    def get_sample_name(self):
        ...
        return "{0}_{1}_{2}_{3}".format(
            splitted_file[0], tomo_name, energy, conf
        )
