        ...
        with XradiaFile(self.xrm_filename) as xrm_file:
            try:
                ...
                self.metadata_h5.create_dataset(
                    "angle", data=self.metadata["angle"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "energy", data=self.metadata["energy"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "exposure_time", data=self.metadata["exposure_time"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "machine_current", data=self.metadata["machine_current"]
                )
                self.metadata_h5["machine_current"].attrs["units"] = "mA"
            except Exception:
                print(
                    "machine_current could not be " "converted from xrm to hdf5"
                )

            try:
                self.metadata["polarisation"] = xrm_file.get_polarisation()[0]
                self.metadata_h5.create_dataset(
                    "polarisation", data=self.metadata["polarisation"]
                )
            except Exception:
                pass
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "pixel_size", data=self.metadata["pixel_size"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "magnification", data=self.metadata["magnification"]
                )
            except Exception:
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "image_width", data=self.metadata["image_width"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "image_height", data=self.metadata["image_height"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "x_position", data=self.metadata["x_position"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "y_position", data=self.metadata["y_position"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "z_position", data=self.metadata["z_position"]
                )
            ...
            try:
                ...
                self.metadata_h5.create_dataset(
                    "data_type", data=self.metadata["data_type"]
                )
            except Exception:
                print("data_type could not be converted from xrm to hdf5")

            try:
                basename = os.path.basename(self.xrm_filename)
                self.metadata["sample_name"] = basename.split("_")[1]
                self.metadata_h5.create_dataset(
                    "sample_name", data=self.metadata["sample_name"]
                )
            except Exception:
                print("sample_name could not be converted from xrm to hdf5")

            try:
                if "_FF" in self.xrm_filename or "_ff_" in self.xrm_filename:
                    self.metadata["FF"] = True
                    self.metadata_h5.create_dataset(
                        "FF", data=self.metadata["FF"]
                    )
                else:
            ...
            try:
                ...
                print(
                    "acquisition date and time could not be converted "
                    "from xrm to hdf5"
                )
            ...
            self.metadata_h5.create_dataset(
                "instrument", data=self.metadata["instrument"]
            )
            self.metadata["source"] = "ALBA"
            self.metadata_h5.create_dataset(
                "source", data=self.metadata["source"]
            )
            self.metadata["source_probe"] = "X-Ray"
            self.metadata_h5.create_dataset(
                "source_probe", data=self.metadata["source_probe"]
            )
            self.metadata["source_type"] = "Sychrotron X-Ray Source"

            # Applied program and command to convert the data
            self.metadata_h5.create_dataset(
                "source_type", data=self.metadata["source_type"]
            )
            self.metadata["program_name"] = "xrm2h5"
            self.metadata_h5.create_dataset(
                "program_name", data=self.metadata["program_name"]
            )
            command = (
                self.metadata["program_name"] + " " + " ".join(sys.argv[1:])
            )
            self.metadata["command"] = command
            self.metadata_h5.create_dataset(
                "command", data=self.metadata["command"]
            )

            # Input and output files
            self.metadata["input_file"] = os.path.basename(self.xrm_filename)
            self.metadata_h5.create_dataset(
                "input_file", data=self.metadata["input_file"]
            )
            self.metadata["output_file"] = os.path.basename(self.h5_filename)
            self.metadata_h5.create_dataset(
                "output_file", data=self.metadata["output_file"]
            )
        ...
        with XradiaFile(self.xrm_filename) as self.xrm_file:
            try:
                ...
                self.h5_handler.create_dataset(
                    "data_1", dtype=np.uint16, data=self.data["data"]
                )
                workflow_step = 1
