...
from .ui_magnetism2Dreconstruction import Ui_magnetism2Dreconstruction
...
class magnetism_2Dreconstruction_GUI(QDialog):
    '''
     Main pyqt5 GUI
    '''
    def __init__(self):
        ...
        '''
        super().__init__()
        self.ui = Ui_magnetism2Dreconstruction() # Open GUI
        ...
        # Connections
        self.ui.selectPath_pushButton.clicked.connect(lambda: self.select_folder_dialog(self.ui.path_lineEdit)) # Open dialog to select path to save.
        self.ui.selectTS1_pushButton.clicked.connect(lambda: self.select_file_dialog(self.ui.TS1_lineEdit,'HDF5 Files (*.hdf5)')) # Open dialog to select HDF5
        self.ui.selectTS2_pushButton.clicked.connect(lambda: self.select_file_dialog(self.ui.TS2_lineEdit,'HDF5 Files (*.hdf5)')) # Open dialog to select HDF5
    ...
    def start_program(self):
        ...
        command = ["magnetism_2Dreconstruction"]
    ...
    def read_arg_values(self):
        ...
        arguments = [self.ui.TS1_lineEdit.text(),self.ui.TS2_lineEdit.text()]
        if self.ui.roiTS1_checkBox.isChecked(): arguments.extend(['--crop_cos'])
        if self.ui.roiTS2_checkBox.isChecked(): arguments.extend(['--crop_rot_cos'])
        if self.ui.selectPointsTS1_checkBox.isChecked(): arguments.extend(['--select_points_cos'])
        if self.ui.selectPointsTS2_checkBox.isChecked(): arguments.extend(['--select_points_rot_cos'])
        if self.ui.roiJoinTS_checkBox.isChecked(): arguments.extend(['--crop_join_tilt'])
        if self.ui.selectPointsJoinTS_checkBox.isChecked(): 
            arguments.extend(['--select_points_join_tilt'])
        else:
             arguments.extend(['--initial_angle_join_tilt',str(self.ui.iniAngleJoinTS_doubleSpinBox.value())])
        arguments.extend(['--attenuation_lenght',self.ui.attLength_lineEdit.text()])
        arguments.extend(['--thickness',self.ui.thickness_lineEdit.text()])
        # arguments.extend(['--start_index_mask',self.ui.idxMask_lineEdit.text()])
        arguments.extend(['--dichroic_coefficient',self.ui.dicCoef_lineEdit.text()])
        if self.ui.repeatStep_comboBox.currentIndex() != 0:
            arguments.extend(['--repeat_step',str(self.ui.repeatStep_comboBox.currentIndex())])
        arguments.extend(['--distance_fiducials',str(self.ui.distFid_doubleSpinBox.value())])
        arguments.extend(['--rot_axis',str(self.ui.rotationAxis_comboBox.currentText())])
...
def main():
    ...
    program = magnetism_2Dreconstruction_GUI()
    program.show()
    sys.exit(app.exec_())
