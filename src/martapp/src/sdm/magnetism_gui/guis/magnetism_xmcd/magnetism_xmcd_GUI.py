...
from PyQt5.QtWidgets import QApplication, QFileDialog, QDialog,QListWidget, QVBoxLayout, QPushButton, QAbstractItemView
...
from .ui_magnetismXMCD import Ui_magnetismXMCD
...
class magnetism_xmcd_GUI(QDialog):
    '''
     Main pyqt5 GUI
    '''
    def __init__(self):
        ...
        '''
        super().__init__()
        self.ui = Ui_magnetismXMCD() # Open GUI
        ...
        # Connections
        self.ui.selectPath_pushButton.clicked.connect(lambda: self.select_folder_dialog(self.ui.path_lineEdit)) # Open dialog to select path.
        self.ui.selectStack1_pushButton.clicked.connect(lambda: self.select_file_dialog(self.ui.stack1_lineEdit,'HDF5 Files (*.hdf5)')) # Open dialog to select HDF5
        self.ui.selectStack2_pushButton.clicked.connect(lambda: self.select_file_dialog(self.ui.stack2_lineEdit,'HDF5 Files (*.hdf5)')) # Open dialog to select HDF5
        ...
        # Disable and enable things
        self.ui.roiSize2DAli_doubleSpinBox.setEnabled(False)
        self.ui.label_14.setEnabled(False)
        self.ui.algorithm2DAli_comboBox.currentIndexChanged.connect(self.disable_algorithm2DAli)
        self.ui.algorithmTiltAli_comboBox.currentIndexChanged.connect(self.disable_algorithmTiltAli)
        self.ui.croppingMethod_comboBox.currentIndexChanged.connect(self.disable_croppingMethod)

        self.thread = None

    def disable_algorithm2DAli(self,idx):
        if idx == 1 or idx==2:
            self.ui.roiSize2DAli_doubleSpinBox.setEnabled(True)
            self.ui.label_14.setEnabled(True)
        else:
            self.ui.roiSize2DAli_doubleSpinBox.setEnabled(False)
            self.ui.label_14.setEnabled(False)

    def disable_algorithmTiltAli(self,idx):
        if idx == 0:
            self.ui.maskTiltAli_checkBox.setEnabled(False)
            self.ui.maskTiltAli_checkBox.setChecked(False)
            self.ui.repeatTiltAlign_checkBox.setEnabled(False)
            self.ui.repeatTiltAlign_checkBox.setChecked(False)
            self.ui.magTiltAli_checkBox.setEnabled(False)
            self.ui.magTiltAli_checkBox.setChecked(False)
        else:
            self.ui.maskTiltAli_checkBox.setEnabled(True)
            self.ui.repeatTiltAlign_checkBox.setEnabled(True)
            self.ui.magTiltAli_checkBox.setEnabled(True)
        if idx == 2:
            self.ui.label.setEnabled(True)
            self.ui.nfid_spinBox.setEnabled(True)
        else:
            self.ui.label.setEnabled(False)
            self.ui.nfid_spinBox.setEnabled(False)

    def disable_croppingMethod(self,idx):
        if idx == 0:
            self.ui.croppingSize_doubleSpinBox.setEnabled(True)
        else:
            self.ui.croppingSize_doubleSpinBox.setEnabled(False)
    ...
    def read_arg_values(self):
        ...
        arguments = [self.ui.stack1_lineEdit.text(),self.ui.stack2_lineEdit.text()]
        arguments.extend(['--pixels_cut_from_borders',str(self.ui.pxCutFromBorders_spinBox.value())])
        if not self.ui.excludeSample_lineEdit.text()=='': arguments.extend(['--exclude_samples',*list(self.ui.excludeSample_lineEdit.text().split(','))]) 
        if self.ui.mask2DAli_checkBox.isChecked(): arguments.extend(['--align_2d_with_mask'])
        if self.ui.roi2DAli_checkBox.isChecked(): arguments.extend(['--align_2d_with_roi'])
        if self.ui.algorithm2DAli_comboBox.currentIndex() == 2:
            arguments.extend(['--align_2d_method','crosscorr'])
            arguments.extend(['--roi_size_2d_align',str(self.ui.roiSize2DAli_doubleSpinBox.value())])
        elif self.ui.algorithm2DAli_comboBox.currentIndex() == 1:
            arguments.extend(['--align_2d_method','crosscorr-fourier'])
        elif self.ui.algorithm2DAli_comboBox.currentIndex() == 3:
            arguments.extend(['--align_2d_method','corrcoeff'])
            arguments.extend(['--roi_size_2d_align',str(self.ui.roiSize2DAli_doubleSpinBox.value())])    
        elif self.ui.algorithm2DAli_comboBox.currentIndex() == 4:
            arguments.extend(['--align_2d_method','of'])
        elif self.ui.algorithm2DAli_comboBox.currentIndex() == 0:
            arguments.extend(['--align_2d_method','pyStackReg'])
        else:
            pass
        if not self.ui.algorithmTiltAli_comboBox.currentIndex() == 0:
            arguments.extend(['--tilt_align']) 
            if  self.ui.maskTiltAli_checkBox.isChecked(): arguments.extend(['--tilt_align_with_mask'])
            if self.ui.algorithmTiltAli_comboBox.currentIndex() == 2: 
                arguments.extend(['--tilt_align_alg','CTalign'])
                arguments.extend(['--nfid',str(self.ui.nfid_spinBox.value())])
            if self.ui.algorithmTiltAli_comboBox.currentIndex() == 1:
                arguments.extend(['--tilt_align_alg','pyStackReg'])
        if self.ui.croppingMethod_comboBox.currentIndex() == 0:
            arguments.extend(['--crop_method','cropping'])
            arguments.extend(['--cropping_percentage',str(self.ui.croppingSize_doubleSpinBox.value())])
        elif self.ui.croppingMethod_comboBox.currentIndex() == 1:
            arguments.extend(['--crop_method','fill'])
        else:
            pass
        if self.ui.repeatTiltAlign_checkBox.isChecked(): arguments.extend(['--repeat_tilt_align'])
        if self.ui.saveToAlign_checkBox.isChecked(): arguments.extend(['--save_to_align'])
        if self.ui.magTiltAli_checkBox.isChecked(): arguments.extend(['--tilt_align_with_mag'])
        if self.ui.subpixelAli_checkBox.isChecked(): arguments.extend(['--subpixel_2d_align'])
    ...
    def close_program(self):
        ...
        '''
        self.close()
        if hasattr(self,'path') and hasattr(self,'arguments'):
    ...
    def visualization(self):
        ...
        '''
        arguments,path = self.read_arg_values()
        if os.path.exists(os.path.join(path,'results.hdf5')):
    ...
    def open_napari_viewer(self, filename, dset, dialog):
        ...
        with h5py.File(filename, "r") as f:
            for i in dset:
                stack = f[i][...]
                stack[stack>1e20] = 0
                viewer.add_image(stack,contrast_limits=(stack.min(),stack.max()),name=i,multiscale=False)
